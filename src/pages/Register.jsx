...
import { Smartphone, ShieldAlert, Sparkles, Mail, Lock, CheckCircle } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password !== confirmPassword) {
      setError('Confirmation mismatch. Password inputs must match.');
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Registration aborted. Email might already be registered.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-[#020617] overflow-hidden text-slate-100 px-6">
      ...
      <div className="w-full max-w-md relative z-10 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2">
          ...
          <h2 className="text-2xl font-black tracking-tight text-white font-display">Create Halo Account</h2>
          <p className="text-xs text-slate-400">Register to connect and synchronize your Android devices</p>
        </div>
        ...
        <div className="glass-card p-8 bg-slate-900/40 border-slate-800/80 rounded-3xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {success && (
              <div className="flex items-center gap-2 p-3.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-400 text-xs">
            ...
            >
              {loading ? (
                ...
                <>
                  <Sparkles className="w-4 h-4" /> Register Console
                </>
              )}
            </button>
          </form>
        ...
        <p className="text-center text-xs text-slate-400">
          ...
          <Link to="/login" className="text-indigo-400 hover:text-indigo-300 font-semibold underline decoration-indigo-400/20">
            Sign In Instead
          </Link>
        </p>
      </div>
